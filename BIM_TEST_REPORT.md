...
- **SSR:** Next 16 forbids `next/dynamic({ ssr: false })` inside Server Components. `src/app/bim-test/page.tsx` is a thin server page that renders `BimTestPageClient.tsx` (`"use client"` + `dynamic(..., { ssr: false })`). Engine code lives under `src/bim-test` and is only imported from that client tree.
